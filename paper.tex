...
Section~\ref{sec:intro} defines the problem statement and contributions.
...
The dataset $\mathcal{D} = \{(\mathbf{x}_t, y_t)\}_{t=1}^{T}$ as defined in
Equation~\eqref{eq:featurevec} consists of
...
\label{eq:cyclic}
\begin{align}
  \text{Hour\_Sin} &= \sin\!\left(\frac{2\pi\cdot\text{Hour}}{24}\right), \label{eq:hoursin}\\
  \text{Hour\_Cos} &= \cos\!\left(\frac{2\pi\cdot\text{Hour}}{24}\right) \label{eq:hourcos}
\end{align}
\end{subequations}
...
would distort~\cite{kashyap2023}. The cyclic encoding in
Equations~\eqref{eq:hoursin} and~\eqref{eq:hourcos} is incorporated into the
final feature vector. The \texttt{Day of the week} column is
...
All rows before \texttt{split\_day} as computed in
Equation~\eqref{eq:split} constitute the training set; all remaining
...
\begin{align}
  ...
  &\quad\to \mathrm{Dense}(K,\text{Softmax}) \label{eq:nnarch}
\end{align}
where $K = 4$ is the number of congestion classes as shown in
Equation~\eqref{eq:nnarch}. Compiled with the Adam
...
Confusion matrices are generated for all 10 (model, setting)
...
Table~\ref{tab:clean} reports accuracy and F1-Macro for each model evaluated
on the clean test set $\mathcal{D}_{\text{test}}$. Histogram Gradient
Boosting achieves the highest accuracy (96.92\%) and F1-Macro (0.9558),
followed closely by Random Forest (95.08\%, F1 = 0.9235) and Neural Network
(94.33\%, F1 = 0.9201). Extra Trees performs competitively (92.92\%, F1 =
0.8859), while Logistic Regression lags as expected for a linear model
(81.25\%, F1 = 0.7836).
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Logistic Regression     & 0.8125 & 0.7836 \\
    Random Forest           & 0.9508 & 0.9235 \\
    Extra Trees             & 0.9292 & 0.8859 \\
    Hist.\ Gradient Boosting & 0.9692 & 0.9558 \\
    Neural Network          & 0.9433 & 0.9201 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Table~\ref{tab:noisy} reports the same metrics for all models evaluated on
the noisy test set $\tilde{\mathcal{D}}_{\text{test}}$, which applies
occlusion ($\pm$3 count perturbation) and sensor dropout (2\% zero-out rate).
All models experience a performance drop, but the magnitude varies
considerably across families. Histogram Gradient Boosting retains the highest
accuracy (92.17\%, F1 = 0.8882), while Logistic Regression degrades least in
relative terms due to its lower clean-setting ceiling. The Neural Network
(91.08\%, F1 = 0.8787) and Random Forest (91.33\%, F1 = 0.8739) remain
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    ...
    \textbf{Model} & \textbf{Accuracy} & \textbf{F1-Macro} \\
    \midrule
    Logistic Regression      & 0.7983 & 0.7677 \\
    Random Forest            & 0.9133 & 0.8739 \\
    Extra Trees              & 0.8983 & 0.8380 \\
    Hist.\ Gradient Boosting  & 0.9217 & 0.8882 \\
    Neural Network           & 0.9108 & 0.8787 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Table~\ref{tab:decay} summarizes the robustness decay ($\Delta F1$,
Equation~\eqref{eq:decay}) for each model, ranked from most to least robust.
Logistic Regression exhibits the smallest absolute decay ($\Delta F1 =
0.0159$), though this reflects its lower clean-data ceiling rather than
genuine robustness. Among the high-performing models, the Neural Network
($\Delta F1 = 0.0414$) demonstrates the best noise tolerance, followed by
Random Forest ($\Delta F1 = 0.0496$). Histogram Gradient Boosting suffers the
largest absolute drop ($\Delta F1 = 0.0676$) despite its top clean-data rank,
suggesting that its aggressive boosting strategy makes it more sensitive to
input perturbations.

\begin{table}[h]
  \centering
  \caption{Robustness Decay: F1-Macro Drop (Clean to Noisy)}
  \label{tab:decay}
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{lccc}
    \toprule
    ...
    \midrule
    Logistic Regression      & 0.7836 & 0.7677 & 0.0159 \\
    Neural Network           & 0.9201 & 0.8787 & 0.0414 \\
    Random Forest            & 0.9235 & 0.8739 & 0.0496 \\
    Extra Trees              & 0.8859 & 0.8380 & 0.0479 \\
    Hist.\ Gradient Boosting  & 0.9558 & 0.8882 & 0.0676 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Confusion matrices for all 10 (model, setting) combinations reveal that the
minority classes \emph{high} (6\% of records) and \emph{low} (14\%) are most
vulnerable to noise-induced misclassification. Class-weighted training in LR,
RF, and ET improves minority-class recall relative to the unweighted HGB
model, particularly for the \emph{high} class. Under noisy conditions, some
\emph{high} instances are misclassified as \emph{normal} across all models,
consistent with the occlusion noise reducing observed vehicle counts and
shifting the feature distribution toward lower-congestion patterns.

\subsection{Discussion}

The results partially confirm the findings of McElfresh et
al.~\cite{mcelfresh2023}: on this tabular traffic dataset, the NN vs.\ GBDT
performance gap is narrow in the clean setting (F1 difference $<$ 0.04
between NN and HGB), consistent with their NeurIPS benchmark. However, the
gap inverts under noise: the Neural Network's Dropout and Batch Normalization
layers confer measurable robustness advantages, as theorized by Ibias et
al.~\cite{ibias2024}. Ensemble averaging in RF and ET provides moderate noise
resistance, though neither reaches the Neural Network's $\Delta F1$.

Compared to the GRU-based digital twin of Abdallah and
Alghamdi~\cite{abdallah2026}, our best clean-setting model (HGB at 96.92\%
accuracy) is competitive on classification accuracy, while our robustness
evaluation framework adds a dimension of operational reliability assessment
absent from their published metrics. The simulation approach proposed here
directly complements their real-time IoT integration architecture.
...
Experimental results show that Histogram Gradient Boosting achieves the
highest clean-data performance (F1-Macro = 0.9558), but the Neural Network
demonstrates superior robustness under sensor degradation ($\Delta F1 =
0.0414$ vs.\ 0.0676 for HGB), suggesting that regularization-aware
architectures are preferable for real-world deployment. Logistic Regression,
while exhibiting the smallest absolute decay, remains unsuitable for
operational use due to its low baseline F1.
